    ...
    setImages()
    {
        ...
        this.images.getResourceAndLoad = (key) =>
        {
            ...
            if(!resource)
            {
                ...
                const textureLoaderType = compressed ? 'textureKtx' : 'texture'
                const loader = this.game.resourcesLoader.getLoader(textureLoaderType)

                let finalKey = key
                if (!compressed && key.endsWith('.ktx')) {
                    finalKey = key.replace(/\.ktx$/, '.png')
                }
                const finalPath = `lab/images/${finalKey}`
            ...
            for(const project of labData)
            {
                ...
                // Image
                {
                    ...
                    mini.startLoading = () =>
                    {
                        ...
                        let finalImageMini = project.imageMini
                        if (!compressed && finalImageMini.endsWith('.ktx')) {
                            finalImageMini = finalImageMini.replace(/\.ktx$/, '.png')
                        }

                        loader.load(
                            `lab/images/${finalImageMini}`,
                            (loadedTexture) =>
                            {
